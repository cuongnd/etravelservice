...
SlickDTP.prototype.get_time = function () {
    var DP = new Date(jQuery("#datepicker").datepicker('getDate'));
    var monthNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    if (this.selected_min == "0") {
        this.selected_min = "00";
    }
    if (this.selected_min == "5") {
        this.selected_min = "05";
    }
    ...
    jQuery('#dt_selected').html(monthNames[DP.getMonth()] + " " + DP.getDate() + ", " + DP.getFullYear() + " @ " + this.selected_hour + ":" + this.selected_min + " " + this.selected_ampm);
...
SlickDTP.prototype.clock_size = function () {
    for (var i = 0; i < 12; i++) {
        ...
        jQuery('#ch' + i).css({top: posy, left: posx, position: 'absolute'});
        jQuery('#cm' + i).css({top: posy, left: posx, position: 'absolute'});
    }
};
SlickDTP.prototype.pickDate = function (MySQLDateId, PrettyId) {
    jQuery('#hour_container').show();
    jQuery('#min_container').hide();
    this.current_id = MySQLDateId;
    this.pretty_id = PrettyId;
    if (jQuery(this.current_id).val() != "") {
        var hitParts = 0;
        var mainParts = jQuery(this.current_id).val().split(" ");
        if (mainParts[0].indexOf("-") > 0) {
            ...
            jQuery('#datepicker').datepicker("setDate", new Date(this.selected_year, this.selected_month - 1, this.selected_day));
            hitParts++;
        }
        if (mainParts[1].indexOf(":") > 0) {
            ...
            if (this.selected_hour > 12) {
                ...
                jQuery('.am_slide').hide();
                jQuery('.pm_slide').show();
                jQuery('.am_slide_l').show();
            }
    ...
    jQuery("#date_time_container").dialog("open");
};
SlickDTP.prototype.dateDone = function () {
    jQuery(this.current_id).val(this.mysql_full_date);
    if (this.pretty_id != '') {
        jQuery(this.pretty_id).val(this.pretty_full_date);
    }
    jQuery("#date_time_container").dialog("close");
};
SlickDTP.prototype.setHourCircle = function (inHour) {
    ...
    jQuery('.circle_hour').removeClass('circle_hour_selected');
    jQuery('#ch' + chid).addClass('circle_hour_selected');
};
...
SlickDTP.prototype.ready = function () {
    ...
    jQuery('.time_part').html(parentThis.get_time());
    jQuery(".circle_hour").click(function () {
        parentThis.selected_hour = jQuery(this).html();
        jQuery('.time_part').html(parentThis.get_time());
        jQuery('#hour_container').hide();
        jQuery('#min_container').show();
        parentThis.setHourCircle(parentThis.selected_hour);
    });
    jQuery(".circle_min").click(function () {
        parentThis.selected_min = jQuery(this).html();
        jQuery('.time_part').html(parentThis.get_time());
        jQuery('#hour_container').show();
        jQuery('#min_container').hide();
        parentThis.setMinCircle(parentThis.selected_min);
    });
    jQuery(".am_slide").click(function () {
        jQuery('.am_slide').toggle();
        jQuery('.pm_slide').toggle();
        jQuery('.am_slide_l').toggle("slow", function () {
        });
        parentThis.selected_ampm = "PM";
        jQuery('.time_part').html(parentThis.get_time());
    });
    jQuery(".pm_slide").click(function () {
        jQuery('.am_slide').toggle();
        jQuery('.pm_slide').toggle();
        jQuery('.am_slide_l').toggle("slow", function () {
        });
    ...
    jQuery("#use_button").click(function () {
        parentThis.dateDone();
    });
    jQuery('#datepicker').datepicker({
    ...
    jQuery("#date_time_container").dialog({height: 420}, {width: 660}, {autoOpen: false},{draggable:true});
};
