 ...
 * @subpackage Currency
 */
class tsmartModeltourstyle extends tmsModel {
	...
	function getListQuery()
	{
		...
		if ($search) {
			...
			$query->where('tour_style.tour_style_name LIKE '.$search);
		}
	...
	function store(&$data){
		if(!vmAccess::manager('tourstyle')){
			vmWarn('Insufficient permissions to store tourstyle');
			return false;
		}
	...
	function remove($ids){
		if(!vmAccess::manager('tourstyle')){
			vmWarn('Insufficient permissions to remove tourstyle');
			return false;
		}
