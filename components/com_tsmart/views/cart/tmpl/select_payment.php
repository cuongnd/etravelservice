...
if (tsmConfig::get('oncheckout_show_steps', 1)) {
	echo '<div class="checkoutStep" id="checkoutStep3">' . tsmText::_('com_tsmart_USER_FORM_CART_STEP3') . '</div>';
}
...
if($this->cart->tsmart_paymentmethod_id){
	echo '<h'.$headerLevel.' class="vm-payment-header-selected">'.tsmText::_('com_tsmart_CART_SELECTED_PAYMENT_SELECT').'</h'.$headerLevel.'>';
} else {
	echo '<h'.$headerLevel.' class="vm-payment-header-select">'.tsmText::_('com_tsmart_CART_SELECT_PAYMENT').'</h'.$headerLevel.'>';
} ?>
