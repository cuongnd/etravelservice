...
<div class="control-buttons">
	...
	if (tsmConfig::get ('oncheckout_show_register', 1) && $view->userDetails->JUser->id == 0 && !tsmConfig::get ('oncheckout_only_registered', 0) && $view->address_type == 'BT' and $rview == 'cart') {
		echo '<div id="reg_text">'.tsmText::sprintf ('com_tsmart_ONCHECKOUT_DEFAULT_TEXT_REGISTER', tsmText::_ ('com_tsmart_REGISTER_AND_CHECKOUT'), tsmText::_ ('com_tsmart_CHECKOUT_AS_GUEST')).'</div>';			}
	else {
		//echo vmText::_('com_tsmart_REGISTER_ACCOUNT');
	}
	if (tsmConfig::get ('oncheckout_show_register', 1) && $view->userDetails->JUser->id == 0 && $view->address_type == 'BT' and $rview == 'cart') {
		...
		<button name="register" class="<?php echo $buttonclass ?>" type="submit" onclick="javascript:return myValidator(userForm,true);"
				title="<?php echo tsmText::_ ('com_tsmart_REGISTER_AND_CHECKOUT'); ?>"><?php echo tsmText::_ ('com_tsmart_REGISTER_AND_CHECKOUT'); ?></button>
		<?php if (!tsmConfig::get ('oncheckout_only_registered', 0)) { ?>
			<button name="save" class="<?php echo $buttonclass ?>" title="<?php echo tsmText::_ ('com_tsmart_CHECKOUT_AS_GUEST'); ?>" type="submit"
					onclick="javascript:return myValidator(userForm, false);"><?php echo tsmText::_ ('com_tsmart_CHECKOUT_AS_GUEST'); ?></button>
		<?php } ?>
		<button class="default" type="reset"
				onclick="window.location.href='<?php echo JRoute::_ ('index.php?option=com_tsmart&view=' . $rview.'&task=cancel'); ?>'"><?php echo tsmText::_ ('com_tsmart_CANCEL'); ?></button>
	<?php
	}
	else {
		?>
		<button class="<?php echo $buttonclass ?>" type="submit"
				onclick="javascript:return myValidator(userForm,true);"><?php echo tsmText::_ ('com_tsmart_SAVE'); ?></button>
		<button class="default" type="reset"
				onclick="window.location.href='<?php echo JRoute::_ ('index.php?option=com_tsmart&view=' . $rview.'&task=cancel'); ?>'"><?php echo tsmText::_ ('com_tsmart_CANCEL'); ?></button>
	<?php } ?>
</div>
...
$url = JRoute::_ ('index.php?option=com_tsmart&view='.$rview.$task, $this->useXHTML, $this->useSSL);

echo shopFunctionsF::getLoginForm (TRUE, FALSE, $url);

?>

<form method="post" id="userForm" name="userForm" class="form-validate" action="<?php echo JRoute::_('index.php?option=com_tsmart&view=user',$this->useXHTML,$this->useSSL) ?>" >
<fieldset>
	<h2><?php
		if ($this->address_type == 'BT') {
			echo tsmText::_ ('com_tsmart_USER_FORM_EDIT_BILLTO_LBL');
		}
		else {
			echo tsmText::_ ('com_tsmart_USER_FORM_ADD_SHIPTO_LBL');
		}
		?>
	</h2>
	...
	if(tsmConfig::get ('reg_captcha')){
		...
		<fieldset id="recaptcha_wrapper" <?php echo $hide_captcha ?>>
			<?php if(!tsmConfig::get ('oncheckout_only_registered')) { ?>
				<span class="userfields_info"><?php echo tsmText::_ ('com_tsmart_USER_FORM_CAPTCHA'); ?></span>
			<?php } ?>
...
<?php }
	...
	<?php if (!empty($this->tsmart_userinfo_id)) {
		echo '<input type="hidden" name="shipto_tsmart_userinfo_id" value="' . (int)$this->tsmart_userinfo_id . '" />';
	}
