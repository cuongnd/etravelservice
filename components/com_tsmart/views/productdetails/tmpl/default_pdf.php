	...
	if (!empty($this->product->customfields)) { ?>
		...
		foreach ($this->product->customfields as $field){
			...
			<?php if ($field->custom_title != $custom_title) { ?>
				<span class="product-fields-title" ><strong><?php echo tsmText::_($field->custom_title); ?></strong></span>
				<?php //echo JHtml::tooltip($field->custom_tip, $field->custom_title, 'tooltip.png');
			} ?>
			<span class="product-field-display"><?php echo $field->display ?></span>
			<span class="product-field-desc"><?php echo tsmText::_($field->custom_desc) ?></span>
			</div>
			<?php
 ...
 ?>
	...
	if($this->allowRating || $this->showReview) {
		...
		for ($num=0 ; $num <= $maxrating; $num++  ) {
			$title = (tsmText::_("VM_RATING_TITLE").' : '. $num . '/' . $maxrating) ;
			$stars[] = JHtml::image($starsPath.$num.'.gif', tsmText::_($num.'_STARS'), array("title" => $title) );
		} ?>
