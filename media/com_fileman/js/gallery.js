...
!function ($) {
    ...
    var Gallery = function(gallery, options){
        ...
        var thumb = this.thumbnails.find('li a .file-thumbnail');
        if(thumb) {
    ...
    Gallery.prototype.setDimensions = function(){
        ...
        if(this._cached_gallery_width != this.thumbnails.width()) {
            ...
            $.each(thumbs, function(row, items) {
                var max_height = 0;
                $.each(items, function(i, item) {
                    var li = item.parents('ol.gallery-thumbnails li');
                    if (li.height() > max_height) max_height = li.height();
                });
                $.each(items, function(i, item) {
                    var li = item.parents('ol.gallery-thumbnails li');
                    li.css('min-height', max_height);
                });
            });
