...
//vmdebug('my query stuff ',$queryArray);
	...
	*/
    ...
    public function store (&$product) {
		...
		$err = $db->getErrorMsg();
		if(!empty($err)){
			vmError('can not delete service class in this tour',$err);
		}
		...
		foreach($list_tour_service_class_id as $virtuemart_service_class_id)
		{
			...
			$err = $db->getErrorMsg();
			if(!empty($err)){
				vmError('can not insert service class in this tour',$err);
			}
		}
		...
		$err = $db->getErrorMsg();
		if(!empty($err)){
			vmError('can not delete group size in this tour',$err);
		}
