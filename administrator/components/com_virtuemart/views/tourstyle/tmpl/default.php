...
if ($saveOrder) {
    ...
    $saveOrderingUrl = 'index.php?option=com_virtuemart&controller=tourstyle&task=saveOrderAjax&tmpl=component';
    JHtml::_('sortablelist.sortable', 'tour_style_list', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
        ...
        <div id="editcell">
            ...
            <table id="tour_style_list" class="adminlist table table-striped" cellspacing="0" cellpadding="0">
                ...
                for ($i = 0, $n = count($this->items); $i < $n; $i++) {
                    ...
                    $checked = JHtml::_('grid.id', $i, $row->virtuemart_tour_style_id);
                    $published = $this->gridPublished($row, $i);

                    $editlink = JROUTE::_('index.php?option=com_virtuemart&view=tourstyle&task=show_parent_popup&cid[]=' . $row->virtuemart_tour_style_id);
                    $edit = $this->gridEdit($row, $i, 'virtuemart_tour_style_id', $editlink);
                    $delete = $this->grid_delete_in_line($row, $i, 'virtuemart_tour_style_id');
                    ?>
