...
$doc->addScript(JUri::root().'/administrator/components/com_virtuemart/assets/js/view_faq_default.js');
...
if ($saveOrder) {
    ...
    $saveOrderingUrl = 'index.php?option=com_virtuemart&controller= faq&task=saveOrderAjax&tmpl=component';
            ...
            <div id="editcell">
                    ...
                    <tr>
                        <th class="admin-checkbox">
                            <label class="checkbox"><input type="checkbox" name="toggle" value=""
                                                           onclick="Joomla.checkAll(this)"/><?php echo $this->sort('virtuemart_faq_id', 'Id'); ?>
                        </th>
                        <th>
				...
				<?php echo vmText::_('COM_VIRTUEMART_SHARED'); ?>
                    ...
                    for ($i = 0, $n = count($this->items); $i < $n; $i++) {
                        ...
                        $checked = JHtml::_('grid.id', $i, $row->virtuemart_faq_id);
                        $published = $this->gridPublished($row, $i);

                        $editlink = JROUTE::_('index.php?option=com_virtuemart&view=faq&task=show_parent_popup&cid[]=' . $row->virtuemart_faq_id);
                        $edit = $this->gridEdit($row, $i, 'virtuemart_tour_type_id', $editlink);
                        $delete = $this->grid_delete_in_line($row, $i, 'virtuemart_ faq_id');
