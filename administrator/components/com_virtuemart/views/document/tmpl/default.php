...
?>
    <script type="text/javascript">
        jQuery(document).ready(function ($) {
            $('.view-document-default').view_document_default({});
        });
    </script>
<?php
...
$doc->addScript(JUri::root().'/administrator/components/com_virtuemart/assets/js/view_document_default.js');
...
if ($saveOrder) {
    ...
    $saveOrderingUrl = 'index.php?option=com_virtuemart&controller=document&task=saveOrderAjax&tmpl=component';
    JHtml::_('sortablelist.sortable', 'tour_type_list', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}


?>
    <div class="view-document-default form-tour-build">
            ...
            <div id="editcell">
                    ...
                    <tr>
                        <th class="admin-checkbox">
                            <label class="checkbox"><input type="checkbox" name="toggle" value=""
                                                           onclick="Joomla.checkAll(this)"/><?php echo $this->sort('virtuemart_document_id', 'Id'); ?>
                        </th>
                        <th>
				...
				<?php echo vmText::_('COM_VIRTUEMART_SHARED'); ?>
                    ...
                    for ($i = 0, $n = count($this->items); $i < $n; $i++) {
                        ...
                        $checked = JHtml::_('grid.id', $i, $row->virtuemart_document_id);
                        $published = $this->gridPublished($row, $i);

                        $editlink = JROUTE::_('index.php?option=com_virtuemart&view=document&task=show_parent_popup&cid[]=' . $row->virtuemart_document_id);
                        $edit = $this->gridEdit($row, $i, 'virtuemart_tour_type_id', $editlink);
                        $delete = $this->grid_delete_in_line($row, $i, 'virtuemart_document_id');
