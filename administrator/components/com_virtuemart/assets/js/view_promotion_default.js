...
/*
            ...
            $element.find('.edit-price').click(function(){
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#tour_methor').val(response.tour.tour_methor);
                        $( "#price-form" ).dialog( "open" );


                        $( "#price-form" ).find('#service_class_id option').css({
                            display:"none"
                        });
                        response.virtuemart_service_class_ids.push(0);
                        $.each(response.virtuemart_service_class_ids, function( index, value ) {
                            $( "#price-form" ).find('#service_class_id option[value="'+value+'"]').css({
                                display:"block"
                            });
                        });
                        var virtuemart_product_id=response.tour.virtuemart_product_id;
                        $("#select_virtuemart_product_id").val(virtuemart_product_id);
                        $("#select_virtuemart_product_id").trigger("liszt:updated");
                        $("#service_class_id").trigger("liszt:updated");
                        $('#template_price').html(response.price_content);
            ...
            $('.dialog-form-price').find('.apply-price').click(function(){
                ...
                if (confirm('Are you sure you want save this item ?')) {
                    ...
                    var $row=$('.list-prices tr[data-price_id="'+promotion_price_id+'"]');
                    //save and close
                    $.ajax({
                        type: "POST",
                        contentType: 'application/json',
            ...
            Joomla.submitbutton = function(task)
            {
                if (task == "add")
                {
                    var virtuemart_product_id=$('input[name="virtuemart_product_id"]').val();
                    $.ajax({
                        type: "GET",
                        url: 'index.php',
                        dataType: "json",
        ...
        plugin.setup_calculator_promotion_price=function(){
            ...
            //validate promotion
            $('table.promotion-price tr.percent input').focusin(function(e){
                total=0;
                $('table.promotion-price tr.amount input').each(function(){
                    total+=$(this).val();
                });
                if(total>0)
                {
            ...
            $('table.promotion-price tr.amount input').focusin(function(e){
                total=0;
                $('table.promotion-price tr.percent input').each(function(){
                    total+=$(this).val();
                });
                if(total>0)
                {
        ...
        plugin.fill_data=function(result)
        {
            ...
            $('input[name="tax"]').val(result.promotion_price.tax);
            $('input[name="tour_methor"]').val(result.tour.tour_methor);
            $('input[name="virtuemart_promotion_price_id"]').val(result.promotion_price.virtuemart_promotion_price_id);
            $('textarea[name="price_note"]').val(result.promotion_price.price_note.trim());
            $('#sale_period_from').val(result.promotion_price.sale_period_from);
            ...
            $('#sale_period_to').val(result.promotion_price.sale_period_to);
        ...
        plugin.updata_price=function updata_price(respone){
            var tour_methor=$('#tour_methor').val();
            var virtuemart_product_id=$('input[name="virtuemart_product_id"]').val();
            var $promotion_price_tr_first= $('#price-form').find('table.base-price tbody tr');
            ...
            $promotion_price_tr_first.find('input').each(function(){
                ...
                var promotion_amount=$('table.promotion-price tr.amount input[column-type="'+column_type+'"]').val();
                ...
                var promotion_percent=$('table.promotion-price tr.percent input[column-type="'+column_type+'"]').val();
