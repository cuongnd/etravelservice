...
window.addEvent('domready', function() {
    ...
    jQuery('#submenu').prependTo('#files-sidebar').addClass('docman-main-nav');
    <? endif ?>

	var sidebar = new Koowa.Sidebar({sidebar: '#files-sidebar', target: '#files-tree'}),
		...
		options = {
            ...
            events: {
                afterSelect: sidebar.setHeight.bind(sidebar),
                afterSetContainer: sidebar.setHeight.bind(sidebar),
                afterSetGrid: sidebar.setHeight.bind(sidebar)
            },
    ...
    //@TODO rewrite Files so that using onAfterXyz events in the options object does not overwrite said events
    // that are added internally in files.js
    ...
    var refresh = document.id('toolbar-refresh');
    if (refresh) {
    	refresh.addEvent('click', function(e) {
            e.stop();
            Files.app.navigate(undefined, 'stateless', true);
        });
    }
});
</script>
...
<div id="files-app">
    <?= @template('com://admin/files.view.files.tmpl.templates_icons'); ?>
	<?= @template('com://admin/docman.view.files.tmpl.app.templates_details'); ?>
	...
	<div id="files-canvas">
		...
		<?= @template('com://admin/docman.view.files.tmpl.app.uploader');?>
	</div>
        ...
        <div class="com_docman">
            <form class="files-modal well">
                <div style="text-align: center;">
                    <h3 style=" float: none">
                        <?= @text('Create a new folder in %folder%', array(
