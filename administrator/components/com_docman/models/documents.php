...
class ComDocmanModelDocuments extends ComDocmanModelDefault
{
    ...
    protected function _buildQuerySearchKeyword(KDatabaseQuery $query)
    {
        ...
        if (!empty($search))
        {
            ...
            {
                case 'exact':
                    $keyword = $this->getTable()->getDatabase()->quoteValue('%'.$search.'%');
                    $query->where[] = array(
                        'property' => '',
                    ...
                    break;
                case 'any':
                    $keyword = $this->getTable()->getDatabase()->quoteValue(implode('|', explode(' ', $search)));
                    $query->where[] = array(
                        'property' => '',
                        'condition' => 'AND '.sprintf('(tbl.title RLIKE %1$s OR tbl.description RLIKE %1$s)', $keyword)
                    );
                    break;
                case 'all':
