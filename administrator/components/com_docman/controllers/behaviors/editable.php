...
class ComDocmanControllerBehaviorEditable extends KControllerBehaviorEditable
{
    ...
    protected function _actionApply(KCommandContext $context)
    {
        ...
        if (JFactory::getApplication()->isSite() && $data instanceof ComDocmanDatabaseRowDocument)
        {
            $url = JRoute::_(sprintf('index.php?option=com_docman&view=document&layout=form&alias=%s&category_slug=%s&Itemid=%d',
                $data->alias, $data->category->slug, $this->getRequest()->Itemid), false);
