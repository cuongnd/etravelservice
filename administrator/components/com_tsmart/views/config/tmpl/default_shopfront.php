...
<table class="admintable">
	...
	echo VmHTML::row('raw','com_tsmart_LWH_UNIT_DEFAULT',ShopFunctions::renderLWHUnitList('lwh_unit_default', VmConfig::get('lwh_unit_default')));
	echo VmHTML::row('checkbox','com_tsmart_ADMIN_SHOW_PRINTICON','show_printicon',VmConfig::get('show_printicon',1));
	echo VmHTML::row('checkbox','com_tsmart_PDF_ICON_SHOW','pdf_icon',VmConfig::get('pdf_icon',0));
?>
</table>
</fieldset>
<fieldset>
...
<?php
	...
	echo VmHTML::row('checkbox','com_tsmart_RECCOMEND_UNATUH','recommend_unauth',VmConfig::get('recommend_unauth',0));
	echo VmHTML::row('checkbox','com_tsmart_ASK_QUESTION_CAPTCHA','ask_captcha',VmConfig::get('ask_captcha',0));
	echo VmHTML::row('checkbox','com_tsmart_ASK_QUESTION_SHOW','ask_question',VmConfig::get('ask_question',0));
	echo VmHTML::row('input','com_tsmart_ASK_QUESTION_MIN_LENGTH','asks_minimum_comment_length',VmConfig::get('asks_minimum_comment_length',50),'class="inputbox"','',4,4);
	echo VmHTML::row('input','com_tsmart_ASK_QUESTION_MAX_LENGTH','asks_maximum_comment_length',VmConfig::get('asks_maximum_comment_length',2000),'class="inputbox"','',5,5);
?>
</table>
</fieldset>
<fieldset>
<legend><?php echo tsmText::_('com_tsmart_COUPONS_ENABLE'); ?></legend>
	<table class="admintable">
		...
		echo VmHTML::row('genericlist','com_tsmart_COUPONS_REMOVE',$this->os_Options,'cp_rm[]',$attrlist, 'order_status_code', 'order_status_name', VmConfig::get('cp_rm',array('C')), 'cp_rm',true);
	?>
	</table>
</fieldset>
<fieldset>
...
<?php
	...
	echo VmHTML::row('checkbox','com_tsmart_DISPLAY_STOCK','display_stock',VmConfig::get('display_stock',1));
	echo VmHTML::row('checkbox','com_tsmart_SHOW_PRODUCT_CUSTOMS','show_pcustoms',VmConfig::get('show_pcustoms',1));
	echo VmHTML::row('checkbox','com_tsmart_UNCAT_PARENT_PRODUCTS_SHOW','show_uncat_parent_products',VmConfig::get('show_uncat_parent_products',0));
	echo VmHTML::row('checkbox','com_tsmart_UNCAT_CHILD_PRODUCTS_SHOW','show_uncat_child_products',VmConfig::get('show_uncat_child_products',0));
...
<td>
	...
	<fieldset>
		...
		<table class="admintable">
			...
			echo VmHTML::row('radioList','com_tsmart_ADMIN_CFG_RATING_SHOW','showRatingFor',VmConfig::get('showRatingFor','all'),$showReviewFor);
			echo VmHTML::row('radioList','com_tsmart_ADMIN_CFG_RATING','ratingMode',VmConfig::get('ratingMode','bought'),$reviewMode);

			$attrlist = 'class="inputbox" multiple="multiple" ';
			echo VmHTML::row('genericlist','com_tsmart_REVIEWS_OS',$this->os_Options,'rr_os[]',$attrlist, 'order_status_code', 'order_status_name', VmConfig::get('rr_os',array('C')), 'rr_os',true);
			?>
