        ...
        <div id="editcell">
            ...
            <table class="adminlist table table-striped" id="servicetype_list" cellspacing="0" cellpadding="0">
                ...
                for ($i = 0, $n = count($this->items); $i < $n; $i++) {
                    ...
                    $checked = JHtml::_('grid.id', $i, $row->tsmart_service_type_id);
                    $published = $this->gridPublished($row, $i);
                    $editlink = JROUTE::_('index.php?option=com_tsmart&view=servicetype&task=edit_in_line&cid[]=' . $row->tsmart_service_type_id);
                    $edit = $this->gridEdit($row, $i, 'tsmart_service_type_id', $editlink);
                    $save_link = JROUTE::_('index.php?option=com_tsmart&view=servicetype&task=save_in_line&cid[]=' . $row->tsmart_service_type_id);
                    $save = $this->grid_save_in_line($row, $i, 'tsmart_service_type_id', $save_link);
                    $delete = $this->grid_delete_in_line($row, $i, 'tsmart_service_type_id');
                    $cancel = $this->grid_cancel_in_line($row, $i, 'tsmart_service_type_id');
                    $show_edit = ($show_edit_in_line == 1 && in_array($row->tsmart_service_type_id, $cid)) || ($show_edit_in_line == 1 && count($cid) == 0 && $i == 0);
                    ...
                    <tr class="row<?php echo $k; ?>">
                        <td class="admin-checkbox">
                            <?php if ($show_edit) { ?>
                                <?php echo VmHTML::inputHidden(array(tsmart_service_type_id => $row->tsmart_service_type_id)); ?>
                                <?php echo $checked ?>
                            <?php } else { ?>
                                <?php echo $checked ?>
                            <?php } ?>
