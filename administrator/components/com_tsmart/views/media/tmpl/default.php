...
<div style="text-align: left;">
 <div class="vm-page-nav">
	...
	<tr>
		...
		<th width="50%"><?php echo vmText::_('com_tsmart_VIEW'); ?></th>
		<th style="min-width:120px;width:15%;"><?php echo vmText::_('com_tsmart_FILES_LIST_FILENAME'); ?></th>
		<th style="min-width:30px;width:1%;max-width:40px;"><?php echo vmText::_('com_tsmart_FILES_LIST_FILETYPE'); ?></th>
	...
	if (count($productfileslist) > 0) {
		...
		foreach ($productfileslist as $key => $productfile) {
			...
			/*	?>
				...
				?>
				<td><?php echo JHtml::_('link', JRoute::_($link, FALSE), $productfile->file_title, array('title' => vmText::_('com_tsmart_EDIT').' '.$productfile->file_title)); ?></td>
				<!-- File role -->
				<td><?php
					//Just to have something, we could make this nicer with Icons
					if(!empty($productfile->file_is_product_image)) echo vmText::_('com_tsmart_'.strtoupper($productfile->file_type).'_IMAGE') ;
					if(!empty($productfile->file_is_downloadable)) echo vmText::_('com_tsmart_DOWNLOADABLE') ;
					if(!empty($productfile->file_is_forSale)) echo  vmText::_('com_tsmart_FOR_SALE');
				...
				<?php
					...
					if(file_exists($fullSizeFilenamePath)){
						...
						$file_alt = vmText::_('com_tsmart_NO_IMAGE_SET').' '.$productfile->file_description;
