        ...
        <div id="editcell">
            ...
            <table class="adminlist table table-striped" cellspacing="0" cellpadding="0">
                ...
                <tr>
                    <th class="admin-checkbox">
                        <label class="checkbox"><input type="checkbox" name="toggle" value=""
                                                       onclick="Joomla.checkAll(this)"/><?php echo $this->sort('tsmart_group_size_id', 'Id'); ?>
                        </label>
                ...
                for ($i = 0, $n = count($this->items); $i < $n; $i++) {
                    ...
                    $checked = JHtml::_('grid.id', $i, $row->tsmart_group_size_id);
                    $published = $this->gridPublished($row, $i);


                    $editlink = JROUTE::_('index.php?option=com_tsmart&view=groupsize&task=edit_in_line&cid[]=' . $row->tsmart_group_size_id);

                    $edit = $this->gridEdit($row, $i, 'tsmart_group_size_id', $editlink);
                    $save = $this->grid_save_in_line($row, $i, 'tsmart_group_size_id');
                    $delete = $this->grid_delete_in_line($row, $i, 'tsmart_group_size_id');
                    $cancel = $this->grid_cancel_in_line($row, $i, 'tsmart_group_size_id');
                    $show_edit = ($show_edit_in_line == 1 && in_array($row->tsmart_group_size_id, $cid)) || ($show_edit_in_line == 1 && count($cid) == 0 && $i == 0);
                    ?>
                    <tr class="row<?php echo $k; ?>">
                        <td class="admin-checkbox">
                        <?php if( $row->type!='flat_price'){ ?>
                                <?php if ($show_edit) { ?>
                                    <?php echo VmHTML::inputHidden(array(tsmart_group_size_id => $row->tsmart_group_size_id)); ?>
                                    <?php echo $checked ?>
                                <?php } else { ?>
                                    <?php echo $checked ?>
                                <?php } ?>
