...
<div style="text-align: left;">
	<table class="adminlist table table-striped" cellspacing="0" cellpadding="0">
		...
		<tr>
			...
			<th width="8%"><?php echo $this->sort ('order_number', 'com_tsmart_ORDER_LIST_NUMBER')  ?></th>
			...
			<th><?php echo $this->sort ('tsmart_order_id', 'com_tsmart_ORDER_LIST_ID')  ?></th>
...
;
			foreach ($this->orderslist as $key => $order) {
				$checked = JHtml::_ ('grid.id', $i, $order->tsmart_order_id);
				?>
			<tr class="row<?php echo $k; ?>">
				...
				<!-- Order id -->
				<?php
				$link = 'index.php?option=com_tsmart&view=orders&task=edit&tsmart_order_id=' . $order->tsmart_order_id;
				?>
				...
				<td>
					<?php
					if ($order->tsmart_user_id) {
						$userlink = JROUTE::_ ('index.php?option=com_tsmart&view=user&task=edit&tsmart_user_id[]=' . $order->tsmart_user_id, FALSE);
				...
				<?php
				/* Print view URL */
				$print_url = juri::root () . 'index.php?option=com_tsmart&view=invoice&layout=invoice&tsmart_order_id=' . $order->tsmart_order_id . '&order_number=' . $order->order_number . '&order_pass=' . $order->order_pass;
				...
				<td style="position:relative;">
					<?php echo JHtml::_ ('select.genericlist', $this->orderstatuses, "orders[" . $order->tsmart_order_id . "][order_status]", 'class="orderstatus_select"', 'order_status_code', 'order_status_name', $order->order_status, 'order_status' . $i, TRUE); ?>
					<input type="hidden" name="orders[<?php echo $order->tsmart_order_id; ?>][current_order_status]" value="<?php echo $order->order_status; ?>"/>
					<input type="hidden" name="orders[<?php echo $order->tsmart_order_id; ?>][coupon_code]" value="<?php echo $order->coupon_code; ?>"/>
					<br/>
					<textarea class="element-hidden vm-order_comment vm-showable" name="orders[<?php echo $order->tsmart_order_id; ?>][comments]" cols="5" rows="5"></textarea>
					<?php echo JHtml::_ ('link', '#', tsmText::_ ('com_tsmart_ADD_COMMENT'), array('class' => 'show_comment')); ?>
				</td>
				<!-- Update -->
				<td><?php echo VmHTML::checkbox ('orders[' . $order->tsmart_order_id . '][customer_notified]', 0) . tsmText::_ ('com_tsmart_ORDER_LIST_NOTIFY'); ?>
					<br/>
					<?php echo VmHTML::checkbox ('orders[' . $order->tsmart_order_id . '][customer_send_comment]', 1) . tsmText::_ ('com_tsmart_ORDER_HISTORY_INCLUDE_COMMENT'); ?>
					<br/>
					<?php echo VmHTML::checkbox ('orders[' . $order->tsmart_order_id . '][update_lines]', 1) . tsmText::_ ('com_tsmart_ORDER_UPDATE_LINESTATUS'); ?>
				</td>
				<!-- Total -->
				<td><?php echo $order->order_total; ?></td>
				<td><?php echo JHtml::_ ('link', JRoute::_ ($link, FALSE), $order->tsmart_order_id, array('title' => tsmText::_ ('com_tsmart_ORDER_EDIT_ORDER_ID') . ' ' . $order->tsmart_order_id)); ?></td>
