...
class TsmartControllerorders extends TsmController
{
    ...
    function ajax_save_passenger_cost_to_excursion()
    {
        ...
        foreach ($list_excursion_cost_per_passenger as $item) {
            ...
            $excursion_addon_passenger_price_order_table->surcharge=$surcharge;
            $excursion_addon_passenger_price_order_table->excursion_discount=$discount!=''?(float)$discount:null;
            if (!$excursion_addon_passenger_price_order_table->jstore(true)) {
                throw new Exception('save error: ' . $room_order_table->getError());
            }
        }
        $response->e = 0;
    ...
    {
        $app = JFactory::getApplication();
        $excursion_addon_order_table = tsmTable::getInstance('excursion_addon_order', 'Table');
        $data = (object)$app->input->getArray();
        $group_excursion_payment=$data->group_excursion_payment;
        $tsmart_order_excursion_addon_id=$data->tsmart_order_excursion_addon_id;
        $excursion_addon_order_table->jload($tsmart_order_excursion_addon_id);
        $response=new stdClass();
    ...
    function ajax_save_group_transfer_current_payment()
    {
        $app = JFactory::getApplication();
        $transfer_addon_order_table = tsmTable::getInstance('transfer_addon_order', 'Table');
        $data = (object)$app->input->getArray();
        $group_transfer_payment=$data->group_transfer_payment;
        $tsmart_order_transfer_addon_id=$data->tsmart_order_transfer_addon_id;
        $type=$data->type;
        $transfer_addon_order_table->jload($tsmart_order_transfer_addon_id);
        $response=new stdClass();
        $transfer_addon_order_table->group_transfer_payment=(float)$group_transfer_payment;
        if (!$transfer_addon_order_table->jstore()) {
    ...
    function change_status_passenger_in_excursion_add_on()
    {
        $app = JFactory::getApplication();
        $excursion_addon_passenger_price_order_table = tsmTable::getInstance('excursion_addon_passenger_price_order', 'Table');
        $data = (object)$app->input->getArray();
        ...
        $tsmart_order_excursion_addon_id=$data->tsmart_order_excursion_addon_id;
        $status=$data->status;
        $excursion_addon_passenger_price_order_table->Jload(array(tsmart_passenger_id=>$tsmart_passenger_id,tsmart_order_excursion_addon_id=>$tsmart_order_excursion_addon_id) );
        $response=new stdClass();
        $excursion_addon_passenger_price_order_table->status=(int)$status;
        if (!$excursion_addon_passenger_price_order_table->Jstore()) {
    ...
    function change_status_group_excursion()
    {
        $app = JFactory::getApplication();
        $excursion_addon_order_table = tsmTable::getInstance('excursion_addon_order', 'Table');
        $data = (object)$app->input->getArray();
        $tsmart_order_excursion_addon_id=$data->tsmart_order_excursion_addon_id;
        $excursion_add_on_status=$data->excursion_add_on_status;
        $excursion_addon_order_table->load($tsmart_order_excursion_addon_id);
        $response=new stdClass();
        $excursion_addon_order_table->status=(int)$excursion_add_on_status;
        if (!$excursion_addon_order_table->store()) {
            throw new Exception('save error: ' . $excursion_addon_order_table->getError());
        }
        $response->e = 0;
        echo json_encode($response);
    ...
    function change_status_group_transfer()
    {
        $app = JFactory::getApplication();
        $transfer_addon_order_table = tsmTable::getInstance('transfer_addon_order', 'Table');
        $data = (object)$app->input->getArray();
        $tsmart_order_transfer_addon_id=$data->tsmart_order_transfer_addon_id;
        $transfer_add_on_status=$data->transfer_add_on_status;
        $transfer_addon_order_table->load($tsmart_order_transfer_addon_id);
        $response=new stdClass();
        $transfer_addon_order_table->status=(int)$transfer_add_on_status;
    ...
    function ajax_save_group_excursion_in_for()
    {
        $app = JFactory::getApplication();
        $group_excursion_addon_order_table = tsmTable::getInstance('excursion_addon_order', 'Table');
        $data = (object)$app->input->getArray();
        ...
        $checkin_date=$data->checkin_date;
        $checkout_date=$data->checkout_date;
        $group_excursion_addon_order_table->load($tsmart_order_excursion_addon_id);
        $response=new stdClass();
        $group_excursion_addon_order_table->checkin_date=JFactory::getDate($checkin_date)->toSql();
        $group_excursion_addon_order_table->checkout_date=JFactory::getDate($checkout_date)->toSql();
        $group_excursion_addon_order_table->terms_condition=$terms_condition;
        $group_excursion_addon_order_table->reservation_notes=$reservation_notes;
        if (!$group_excursion_addon_order_table->store()) {
            throw new Exception('save error: ' . $group_excursion_addon_order_table->getError());
        }
        $list_assign_user_id_manager_hotel_add_on=$data->list_assign_user_id_manager_hotel_add_on;
        $db=JFactory::getDbo();
        $order_helper = TSMHelper::getHepler('orders');
        $query=$db->getQuery(true);
        $query->delete('#__tsmart_order_user_manager')
            ->where('object_id='.(int)$tsmart_order_excursion_addon_id)
            ->where('type='.$query->q($order_helper::TYPE_GROUP_EXCURSION_ADD_ON_ORDER))
            ;
        $db->setQuery($query);
        if (!$db->execute()) {
            throw new Exception('save error: ' . $db->getError());
        ...
        foreach($list_assign_user_id_manager_hotel_add_on as $user_id){
            $query->clear();
            $query->insert('#__tsmart_order_user_manager')
                ->set('type ='.$query->q($order_helper::TYPE_GROUP_EXCURSION_ADD_ON_ORDER))
                ->set('user_id='.(int)$user_id)
                ->set('object_id='.(int)$tsmart_order_excursion_addon_id)
            ;
            $db->setQuery($query);
            if (!$db->execute()) {
                throw new Exception('save error: ' . $db->getError());
    ...
    function ajax_get_main_tour_order_detail_by_order_id()
    {
        $app = JFactory::getApplication();
        $order_table = tsmTable::getInstance('orders', 'Table');
        $data = (object)$app->input->getArray();
        ...
        $db=JFactory::getDbo();
        $query=$db->getQuery(true);
        $query->select('main_tour_order.*')
            ->from('#__tsmart_main_tour_order AS main_tour_order')
            ->where('tsmart_order_main_tour_id='.(int)$tsmart_order_main_tour_id)
            ;
        $main_tour_order=$db->setQuery($query)->loadObject();
        $response->main_tour_order=$main_tour_order;
        $response->list_assign_user_id_manager_main_tour=$orders_helper->get_list_assign_user_id_manager_main_tour($tsmart_order_main_tour_id);
    ...
    function ajax_get_order_detail_and_night_hotel_detail_by_hotel_addon_id()
    {
        ...
        $query->select(
            ...
            group_hotel_addon_order.group_hotel_current_discount,
            group_hotel_addon_order.group_hotel_current_commission,
    ...
    function ajax_get_order_detail_and_transfer_add_on_detail_by_tsmart_order_transfer_addon_id()
    {
        ...
        $query->select(
            ...
            transfer_addon_order.adult_cost,
            transfer_addon_order.children_cost,
            transfer_addon_order.children_under_year,
            transfer_addon_order.net_price,
            transfer_addon_order.group_transfer_payment,
            transfer_addon_order.mark_up,
            transfer_addon_order.tax,
        ...
        foreach ($list_passenger as &$passenger_in_transfer) {
            ...
            if($type=='pre') {
                $passenger_in_transfer->pre_transfer_total_cost = (float)$passenger_in_transfer->pre_transfer_fee +(float)$passenger_in_transfer->pre_transfer_surcharge- (float)$passenger_in_transfer->pre_transfer_discount;
                $passenger_in_transfer->pre_transfer_balance = (float)$passenger_in_transfer->pre_transfer_total_cost - (float)$passenger_in_transfer->pre_transfer_payment;
                $passenger_in_transfer->pre_transfer_refund = (float)$passenger_in_transfer->pre_transfer_payment -(float) $passenger_in_transfer->pre_transfer_cancel_fee;
                ...
                $passenger_in_transfer->post_transfer_total_cost = (float)$passenger_in_transfer->post_transfer_fee +(float)$passenger_in_transfer->post_transfer_surcharge - (float)$passenger_in_transfer->post_transfer_discount;
                $passenger_in_transfer->post_transfer_balance = (float)$passenger_in_transfer->post_transfer_total_cost - (float)$passenger_in_transfer->post_transfer_payment;
                $passenger_in_transfer->post_transfer_refund = (float)$passenger_in_transfer->post_transfer_payment - (float)$passenger_in_transfer->post_transfer_cancel_fee;
    ...
    function ajax_get_order_detail_and_excurstion_add_on_detail_by_tsmart_order_excurstion_addon_id()
    {
        ...
        $query->select(
            ...
            excursion_addon_order.adult_cost,
            excursion_addon_order.group_excursion_payment,
            excursion_addon_order.children_cost,
            excursion_addon_order.net_price,
            excursion_addon_order.mark_up,
            excursion_addon_order.tax,
            excursion_addon_order.children_under_year,
        ...
        foreach ($list_passenger as &$passenger_in_excursion) {
            $passenger_in_excursion->excursion_surcharge=$passenger_in_excursion->surcharge;
            $passenger_in_excursion->excursion_total_cost = $passenger_in_excursion->excursion_fee+$passenger_in_excursion->surcharge - $passenger_in_excursion->excursion_discount;
    ...
    function ajax_save_passenger_cost()
    {
        ...
        foreach ($list_row as $row) {
            ...
            $passenger_table->surcharge=$row->surcharge!=''?$row->surcharge:0;
    ...
    function ajax_save_passenger_cost_hotel_add_on()
    {
        ...
        foreach ($list_row as $row) {
            $row=(object)$row;
            $passenger_table->jload($row->tsmart_passenger_id);
            $passenger_table->{$type."_night_discount"}=$row->discount!=''?$row->discount:null;
            $passenger_table->{$type."_night_payment"}=$row->payment!=''?$row->payment:null;
            $passenger_table->{$type."_night_surcharge"}=$row->surcharge!=''?$row->surcharge:0;
            $passenger_table->{$type."_night_cancel_fee"}=$row->cancel_fee!=''?$row->cancel_fee:null;
            if (!$passenger_table->jstore(true)) {
                throw new Exception('save error: ' . $passenger_table->getError());
            }
        }
    ...
    function ajax_save_passenger_cost_excursion_add_on()
    {
        ...
        foreach ($list_row as $row) {
            ...
            $excursion_addon_passenger_price_order_table->excursion_discount=$row->excursion_discount!=''?$row->excursion_discount:null;
            $excursion_addon_passenger_price_order_table->surcharge=$row->excursion_surcharge!=''?$row->excursion_surcharge:null;
            $excursion_addon_passenger_price_order_table->excursion_payment=$row->excursion_payment!=''?$row->excursion_payment:null;
            $excursion_addon_passenger_price_order_table->excursion_cancel_fee=$row->excursion_cancel_fee!=''?$row->excursion_cancel_fee:null;
            if (!$excursion_addon_passenger_price_order_table->jstore(true)) {
                throw new Exception('save error: ' . $excursion_addon_passenger_price_order_table->getError());
            }
        }
    ...
    function ajax_save_passenger_cost_transfer_add_on()
    {
        ...
        foreach ($list_row as $row) {
            $row=(object)$row;
            $passenger_table->jload($row->tsmart_passenger_id);
            $passenger_table->{$type."_transfer_discount"}=$row->discount!=''?$row->discount:null;
            $passenger_table->{$type."_transfer_surcharge"}=$row->surcharge!=''?$row->surcharge:null;
            $passenger_table->{$type."_transfer_payment"}=$row->payment!=''?$row->payment:null;
            $passenger_table->{$type."_transfer_cancel_fee"}=$row->cancel_fee!=''?$row->cancel_fee:null;
            if (!$passenger_table->jstore(true)) {
                throw new Exception('save error: ' . $passenger_table->getError());
            }
        }
    ...
    function ajax_save_main_tour_order_info()
    {
        $app = JFactory::getApplication();
        $main_tour_order_table = tsmTable::getInstance('main_tour_order', 'Table');
        $data = (object)$app->input->getArray();
        ...
        $tsmart_order_main_tour_id=$data->tsmart_order_main_tour_id;
        $main_tour_order_table->jload($tsmart_order_main_tour_id);
        $main_tour_order_table->terms_condition = $terms_condition;
        $main_tour_order_table->reservation_notes = $reservation_notes;
        $main_tour_order_table->itinerary = $itinerary;

        if (!$main_tour_order_table->store()) {
            $response->e = 1;
            $response->m = $main_tour_order_table->getError();
            echo json_encode($response);
            die;
        }
        $order_table = tsmTable::getInstance('orders', 'Table');
        ...
        $list_assign_user_id_manager_main_tour=$data->list_assign_user_id_manager_main_tour;
        $db=JFactory::getDbo();
        $order_helper = TSMHelper::getHepler('orders');
        $query=$db->getQuery(true);
        $query->delete('#__tsmart_order_user_manager')
            ->where('object_id='.(int)$tsmart_order_main_tour_id)
            ->where('type='.$query->q($order_helper::TYPE_GROUP_MAIN_TOUR_ORDER))
        ;
        $db->setQuery($query);
        if (!$db->execute()) {
            throw new Exception('save error: ' . $db->getError());
        ...
        foreach($list_assign_user_id_manager_main_tour as $user_id){
            $query->clear();
            $query->insert('#__tsmart_order_user_manager')
                ->set('type ='.$query->q($order_helper::TYPE_GROUP_MAIN_TOUR_ORDER))
                ->set('user_id='.(int)$user_id)
                ->set('object_id='.(int)$tsmart_order_main_tour_id)
            ;
            $db->setQuery($query);
            if (!$db->execute()) {
                throw new Exception('save error: ' . $db->getError());
