    ...
    $.select_trip_join_and_private = function (element, options) {
        ...
        plugin.update_select_service_class = function (list_service_class) {
            ...
            for (var i = 0; i < list_service_class.length; i++) {
                ...
                $element.find('.list_service_class label[data-tsmart_service_class_id="' + service_class.tsmart_service_class_id + '"]').show();
            }
        };
        plugin.update_select_departure = function (list_departure) {
            ...
            for (var i = 0; i < list_departure.length; i++) {
                ...
                $element.find('.list_departure .departure-item[data-tsmart_departure_id="' + departure.tsmart_departure_id + '"]').show();
            }
        };
        plugin.add_event_change_tour_type = function () {
            ...
            $element.find('select.tour_type').change(function () {
                ...
                var tsmart_tour_type_id = $(this).val();
                if (tsmart_tour_type_id == 2) {
                    $element.find('.air-service-class').show();
                    $element.find('.air-departure').hide();
                } else if (tsmart_tour_type_id == 5) {
                ...
                $.ajax({
                    ...
                    data: (function () {
                        ...
                        dataPost = {
                            ...
                            tsmart_tour_type_id: tsmart_tour_type_id
                        };
                        return dataPost;
                    })(),
                    beforeSend: function () {
        ...
        plugin.add_event_change_tour = function () {
            $element.find('.list_tour .checkbox.tsmart_product_id').change(function () {
                var tsmart_tour_type_id = $element.find('select.tour_type').val();
                if (tsmart_tour_type_id == 2) {
