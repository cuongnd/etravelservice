(function ($) {
    ...
    $.html_build_excursion_addon = function (element, options) {
            ...
            basic: {
                ...
                validate_transfer: function (transfer_item, transfer_index) {
                    ...
                    if (passengers.length > max_total) {
                        var content_notify = 'our policy does not allow two persons to stay in a basic transfer. You are suggested to select a twin/double transfer';
                        ...
                        return false;
                    } else if (plugin.exists_infant_in_transfer_index(transfer_index) || plugin.exists_children_2_in_transfer_index(transfer_index)) {
        ...
        plugin.calculator_tour_cost_and_transfer_price = function () {
            ...
            var price_private_transfer = virtuemart_promotion_price_id ? sale_promotion_price_private_transfer : sale_price_private_transfer;
            ...
            for (var i = 0; i < list_transfer.length; i++) {
                ...
                var func_set_tour_cost_and_transfer_price = function (transfer_item, passenger_index, tour_cost, transfer_price, extra_bed_price,msg,bed_note) {
                    var item_passenger = {};
                    item_passenger.passenger_index = passenger_index;
                    item_passenger.tour_cost = tour_cost;
                    item_passenger.transfer_price = transfer_price;
                ...
                if (transfer_type == "basic") {
                    if (passengers.length == 1 && plugin.get_total_senior_adult_teen(transfer_index) == 1) {
                        //case 0
                        console.log("case 0");
                        var msg="The tour cost for this passenger is based on transfer for 2 persons.  Therefore he or she is required to pay the transfer supplement fee while taking a private transfer.";
                        var passenger_index = plugin.get_senior_adult_teen_passenger_order_in_transfer(transfer_index, 0);
                        var tour_cost = plugin.get_price_tour_cost_by_passenger_index(passenger_index, price_senior, price_adult, price_teen);
                        var transfer_price = price_private_transfer;
                        var bed_note=plugin.settings.private_bed;
                        transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,msg,bed_note);

                    } else if (plugin.get_total_children_1(transfer_index) == 1) {
                        //case 1a
                        console.log("case 1a");
                        if(full_charge_children1){
                            var msg="The tour cost for this passenger is based on transfer for 2 persons.  Therefore he or she is required to pay the basic transfer supplement fee while taking a private transfer.";
                            var passenger_index = plugin.get_children_1_passenger_order_in_transfer(transfer_index, 0);
                            var tour_cost = price_children1;
                            var transfer_price = price_private_transfer ;
                            var bed_note=plugin.settings.private_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,msg,bed_note);
                        }else{
                            var msg="The tour cost for this child type does not include the transfer fee.  Therefore he or she is required to pay the  transfer supplement fee while taking a private transfer.";
                            var passenger_index = plugin.get_children_1_passenger_order_in_transfer(transfer_index, 0);
                            var tour_cost = price_children1;
                            var transfer_price = price_private_transfer * 2;
                            var bed_note=plugin.settings.private_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,msg,bed_note);

                        }
                    }
                } else if (transfer_type == "double" || transfer_type == "twin") {
                    //Double OR Twin
                    console.log('case transfer_type == "double" || transfer_type == "twin" ');
                    if (passengers.length == 2 && plugin.get_total_senior_adult_teen(transfer_index) == 2) {
                        ...
                        var bed_note=plugin.settings.private_bed;
                        transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,"",bed_note);

                        var passenger_index = plugin.get_senior_adult_teen_passenger_order_in_transfer(transfer_index, 1);
                        var tour_cost = plugin.get_price_tour_cost_by_passenger_index(passenger_index, price_senior, price_adult, price_teen);
                        var transfer_price = 0;
                        var bed_note=plugin.settings.private_bed;
                        transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,"",bed_note);
                    ...
                    } else if (passengers.length == 2 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_children_1(transfer_index) == 1) {
                        ...
                        if (full_charge_children1) {
                            ...
                            var tour_cost = price_children1;
                            var transfer_price = price_private_transfer;
                            var msg="The tour cost for this child type does not include any transfer fee. Therefore if you assign this shild to a twin/double sharing transfer, he or she is required to pay half transfer fee.";
                            var bed_note=plugin.settings.private_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,msg,bed_note);

                        }

                    } else if (passengers.length == 2 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_infant_and_children_2(transfer_index) == 1) {
                        //case 2b Adult,1 Child ( 0 - 5 years)
                        //add adult first
                        console.log("case 2b");
                        var msg="This passenger is required to pay the transfer supplement fee to grant the free stay to kid sharing this transfer.";
                        var passenger_index = plugin.get_senior_adult_teen_passenger_order_in_transfer(transfer_index, 0);
                         var tour_cost = plugin.get_price_tour_cost_by_passenger_index(passenger_index, price_senior, price_adult, price_teen);
                        var transfer_price = price_private_transfer;
                        var bed_note=plugin.settings.private_bed;
                        ...
                        for (var p_index = 0; p_index < passengers.length; p_index++) {
                            ...
                            if (plugin.is_infant(a_passenger_index)) {
                                ...
                                transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, a_passenger_index, tour_cost, transfer_price,0,"",bed_note);
                    ...
                    } else if (passengers.length == 2 && plugin.get_total_children_1(transfer_index) == 2) {
                        //2 Child ( 6 - 11 years) (case 2c)
                        console.log("case 2c");
                        for (var p_index = 0; p_index < 2; p_index++) {
                            if (full_charge_children1) {
                                var passenger_index = plugin.get_children_1_passenger_order_in_transfer(transfer_index, p_index);
                                var tour_cost = price_children1;
                                var transfer_price = 0;
                                var bed_note=plugin.settings.private_bed;
                                transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,"",bed_note);
                    ...
                    } else if (passengers.length == 2 && plugin.get_total_children_1(transfer_index) == 1 && plugin.get_total_children_2(transfer_index) == 1) {
                        ...
                        var passenger_index = plugin.get_infant_or_children_2_passenger_order_in_transfer(transfer_index, 0);
                        var tour_cost = price_children2;
                        var transfer_price = price_private_transfer;
                        var msg="include any transfer fee. Therefore if you assign this shild to a twin/double sharing transfer, he or she is required to pay half transfer fee.";
                        var bed_note=plugin.settings.private_bed;
                        transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,msg,bed_note);


                    } else if (passengers.length == 3 && plugin.get_total_senior_adult_teen(transfer_index) == 2 && plugin.get_total_infant_and_children_2(transfer_index) == 1) {
                    ...
                    } else if (passengers.length == 3 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_children_1(transfer_index) == 1 && plugin.get_total_infant_and_children_2(transfer_index) == 1) {
                    ...
                    } else if (passengers.length == 3 && plugin.get_total_children_1(transfer_index) == 2 && plugin.get_total_children_2(transfer_index) == 1) {
                        ...
                        var passenger_index = plugin.get_children_2_passenger_order_in_transfer(transfer_index, 0);
                        var tour_cost = price_children2;
                        var transfer_price = 0;
                        var bed_note=plugin.settings.share_bed;
                        transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,"",bed_note);
                    ...
                    }else if (passengers.length == 3 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_infant_and_children_2(transfer_index) == 2) {
                        ...
                        for (var p_index = 0; p_index < passengers.length; p_index++) {
                            ...
                            if (plugin.is_infant(a_passenger_index)) {
                                ...
                                var bed_note=plugin.settings.share_bed;
                                transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, a_passenger_index, tour_cost, transfer_price, extra_bed_price,msg,bed_note);
                            } else if (plugin.is_children_2(a_passenger_index)) {
                                var tour_cost = price_children2;
                                var transfer_price = 0;
                                var extra_bed_price = price_extra_bed;
                ...
                } else if (transfer_type == "triple") {
                    if (passengers.length == 3 && plugin.get_total_senior_adult_teen(transfer_index) == 3) {
                        //3 adult (case 0)
                        console.log("case 0");
                        var list_bed_note=[];
                        ...
                        for (var p_index = 0; p_index < passengers.length; p_index++) {
                            ...
                            var transfer_price = 0;
                            var bed_note=plugin.settings.private_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, a_passenger_index, tour_cost, transfer_price,0,"",list_bed_note[p_index]);
                        }
                    } else if (passengers.length == 3 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_children_1(transfer_index) == 2) {
                        ...
                        var  list_all_children_1_in_transfer=plugin.get_all_children_1_in_transfer(transfer_index);
                        var list_object_children_1_passenger=plugin.sorting_passengers_by_year_old(list_all_children_1_in_transfer);
                        ...
                        if (full_charge_children1) {
                            ...
                            var tour_cost = price_children1;
                            var transfer_price = price_private_transfer;
                            var msg="not include any transfer fee. Therefore if you assign this shild to a triple sharing transfer, he or she is required to pay half transfer fee.";
                            var bed_note=plugin.settings.private_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,msg,bed_note);
                        ...
                        if (full_charge_children1) {
                            ...
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price, extra_bed_price,0,"",bed_note);
                        } else {
                            var tour_cost = price_children1;
                            var transfer_price = 0;
                            var extra_bed_price = price_extra_bed;
                            var msg="The tour cost for this child type does not include any transfer fee. Therefore if you assign this shild to a triple  sharing transfer, he or she is required to pay extra bed fee.";
                            var bed_note=plugin.settings.extra_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price, extra_bed_price,msg,bed_note);
                        ...
                        if (full_charge_children1) {
                            ...
                            var tour_cost = price_children1;
                            var transfer_price = 0;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price);
                        } else {
                            var tour_cost = price_children1;
                            var transfer_price = price_private_transfer;
                            var msg="not include any transfer fee. Therefore if you assign this shild to a triple sharing transfer, he or she is required to pay half transfer fee.";
                        ...
                        var list_all_infant_children_2_in_transfer=plugin.get_all_infant_children_2_in_transfer(transfer_index);
                        var list_object_infant_children_2_passenger=plugin.sorting_passengers_by_year_old(list_all_infant_children_2_in_transfer);
                        ...
                        var msg="The tour cost for this child type does not include any transfer fee. Therefore  he or she is required to pay extra bed fee while  staying in triple transfer.";
                    ...
                    } else if (passengers.length == 3 && plugin.get_total_senior_adult_teen(transfer_index) == 2 && plugin.get_total_children_1(transfer_index) == 1) {
                        ...
                        if (full_charge_children1) {
                            ...
                            var extra_bed_price = 0;
                            var bed_note=plugin.settings.extra_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price, extra_bed_price,"",bed_note);
                        } else {
                            var msg="The tour cost for this child type does not include any transfer fee. Therefore  he or she is required to pay extra bed fee while  staying in triple transfer.";
                            var tour_cost = price_children1;
                            var transfer_price = 0;
                    ...
                    } else if (passengers.length == 3 && plugin.get_total_children_1(transfer_index) == 3) {
                        ...
                        if (full_charge_children1) {
                            for (var p_index = 0; p_index < passengers.length; p_index++) {
                                ...
                                var extra_bed_price = 0;
                                var bed_note=plugin.settings.private_bed;
                                transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, a_passenger_index, tour_cost, transfer_price, extra_bed_price,"",bed_note);
                            }
                        } else {
                            var msgs=[];
                            msgs[0]="The tour cost for this child type does not include the transfer fee.  Therefore he or she is required to pay the  transfer supplement fee while sharing transfer with other.";
                            msgs[1]="The tour cost for this child type does not include the transfer fee.  Therefore he or she is required to pay the  transfer supplement fee while sharing transfer with other.";
                            msgs[2]="The tour cost for this child type does not include any transfer fee. Therefore  he or she is required to pay extra bed fee while  staying in triple transfer.";
                            ...
                            for (var p_index = 0; p_index < list_object_children_1_passenger.length; p_index++) {
                                ...
                                if (p_index == 0 || p_index == 1) {
                                    transfer_price = price_private_transfer;
                                    extra_bed_price = 0;
                                } else {
                                    transfer_price = 0;
                                    extra_bed_price = price_extra_bed;
                                }
                                transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, a_passenger_index, tour_cost, transfer_price, extra_bed_price,msgs[i],list_bed_note[p_index]);
                        ...
                        if (full_charge_children1) {
                            ...
                            var passenger_index = plugin.get_children_1_passenger_order_in_transfer(transfer_index, 1);
                            var tour_cost = price_children1;
                            var transfer_price = 0;
                            var bed_note=plugin.settings.private_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,"",bed_note);
                            ...
                            var msg="The tour cost for this child type does not include the transfer fee.  Therefore he or she is required to pay the  transfer supplement fee while sharing transfer with other.";
                    ...
                    }else if(passengers.length == 3 && plugin.get_total_children_1(transfer_index) == 1 && plugin.get_total_children_2(transfer_index) == 2){
                        ...
                        var list_children_2_in_transfer=plugin.get_all_children_2_in_transfer(transfer_index);
                        var list_object_children_2_passenger=plugin.sorting_passengers_by_year_old(list_children_2_in_transfer);
                        var msg="The tour cost for this child type does not include the transfer fee.  Therefore he or she is required to pay the  transfer supplement fee while sharing transfer with other.";
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_children_1(transfer_index) == 3) {
                        ...
                        if (full_charge_children1) {
                            ...
                            for (var p_index = 0; p_index < list_object_children_1_passenger.length; p_index++) {
                                ...
                                transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, a_passenger_index, tour_cost, transfer_price, extra_bed_price,msgs[p_index],list_bed_note[p_index]);
                            }
                        }

                    } else if (passengers.length == 4 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_children_1(transfer_index) == 2 && plugin.get_total_infant_and_children_2(transfer_index) == 1) {
                        ...
                        if (full_charge_children1) {
                            ...
                            var extra_bed_price = price_extra_bed;
                            var bed_note=plugin.settings.extra_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,price_extra_bed,msg,bed_note);
                        }
                        // 1 children 0-5
                        for (var p_index = 0; p_index < passengers.length; p_index++) {
                            var a_passenger_index = passengers[p_index];
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_children_1(transfer_index) == 1 && plugin.get_total_infant_and_children_2(transfer_index) == 2) {
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_senior_adult_teen(transfer_index) == 1 && plugin.get_total_infant_and_children_2(transfer_index) == 3) {
                        ...
                        for (var p_index = 0; p_index < list_object_infant_children_2_passenger.length; p_index++) {
                            ...
                            if (p_index == 0 || p_index == 1) {
                                ...
                                var extra_bed_price = price_private_transfer;
                            }
                            ...
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, a_passenger_index, tour_cost, transfer_price, extra_bed_price,msg,list_bed_note[p_index]);
                        }
                    } else if (passengers.length == 4 && plugin.get_total_senior_adult_teen(transfer_index) == 2 && plugin.get_total_children_1(transfer_index) == 2) {
                        ...
                        if (full_charge_children1) {
                            ...
                            var list_all_children_1_in_transfer=plugin.get_all_children_1_in_transfer(transfer_index);
                            var list_children_1_passenger=plugin.sorting_passengers_by_year_old(list_all_children_1_in_transfer);
                            ...
                            var tour_cost = price_children1;
                            var transfer_price = (price_private_transfer+price_extra_bed)/2;
                            var extra_bed_price = 0;
                            var bed_note=plugin.settings.extra_bed;
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price, extra_bed_price,msg,bed_note);
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_senior_adult_teen(transfer_index) == 2 && plugin.get_total_children_1(transfer_index) == 1 && plugin.get_total_infant_and_children_2(transfer_index) == 1) {
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_senior_adult_teen(transfer_index) == 2  && plugin.get_total_infant_and_children_2(transfer_index) == 2) {
                        ...
                        var list_all_infant_children_2_in_transfer=plugin.get_all_infant_children_2_in_transfer(transfer_index);
                        var list_infant_children_2_passenger=plugin.sorting_passengers_by_year_old(list_all_infant_children_2_in_transfer);
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_senior_adult_teen(transfer_index) == 3  && plugin.get_total_infant_and_children_2(transfer_index) == 1) {
                        ...
                        //adult 3
                        var passenger_index = plugin.get_senior_adult_teen_passenger_order_in_transfer(transfer_index, 2);
                         var tour_cost = plugin.get_price_tour_cost_by_passenger_index(passenger_index, price_senior, price_adult, price_teen);
                        var transfer_price = 0;
                        var bed_note=plugin.settings.extra_bed;
                        transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price,0,"",bed_note);
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_children_1(transfer_index) == 4) {
                        ...
                        if (full_charge_children1) {
                            ...
                            msgs[0]="The tour cost for this child type does not include the transfer fee.  Therefore he or she is required to pay the  transfer supplement fee while sharing transfer with other.";
                            msgs[1]="The tour cost for this child type does not include the transfer fee.  Therefore he or she is required to pay the  transfer supplement fee while sharing transfer with other.";
                            msgs[2]="The tour cost for this child type does not include the transfer fee.  Therefore he or she is required to pay the  transfer supplement fee while sharing transfer with other.";
                            msgs[3]="The tour cost for this child type does not include any transfer fee. Therefore  he or she is required to pay extra bed fee while  staying in triple transfer.";
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_children_1(transfer_index) == 3 && plugin.get_total_children_2(transfer_index) == 1) {
                    ...
                    } else if (passengers.length == 4 && plugin.get_total_children_1(transfer_index) == 2 && plugin.get_total_infant_and_children_2(transfer_index) == 2) {
                        ...
                        var list_all_children_2_in_transfer=plugin.get_all_children_2_in_transfer(transfer_index);
                        var list_object_children_2_passenger=plugin.sorting_passengers_by_year_old(list_all_children_2_in_transfer);
                        ...
                        var bed_note=plugin.settings.share_bed;
                        transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, passenger_index, tour_cost, transfer_price, extra_bed_price,"",bed_note);
                    } else if (passengers.length == 4 && plugin.get_total_children_1(transfer_index) == 1 && plugin.get_total_children_2(transfer_index) == 3) {
                        ...
                        if (full_charge_children1) {
                            ...
                            var msg="The tour cost for this child type does not include any transfer fee. Therefore if you assign this shild to this triple transfer, he or she is required to pay the transfer supplement fee.";
                            var passenger_index = plugin.get_children_1_passenger_order_in_transfer(transfer_index, 0);
                            var tour_cost = price_children1;
                            var transfer_price = price_private_transfer;
                            var extra_bed_price = 0;
                        ...
                        //3 children 2-5
                        var list_msg=[];
                        list_msg[0]="The tour cost for this child type does not include any transfer fee. Therefore if you assign this shild to this triple transfer, he or she is required to pay the transfer supplement fee.";
                        list_msg[1]="The tour cost for this child type does not include any transfer fee. Therefore  he or she is required to pay extra bed fee while  staying in triple transfer.";
                        list_msg[2]="";
                        ...
                        for (var p_index = 0; p_index < list_object_children_2_passenger.length; p_index++) {
                            ...
                            transfer_item = func_set_tour_cost_and_transfer_price(transfer_item, a_passenger_index, tour_cost, transfer_price, extra_bed_price,list_msg[p_index],list_bed_note[p_index]);
        ...
        plugin.lock_passenger_inside_transfer_index = function (transfer_index) {
            ...
            if(list_passenger_selected.length>0)
            {
                for (var i = 0; i < list_passenger_selected.length; i++) {
                    ...
                    console.log("a_transfer_index:"+a_transfer_index);
