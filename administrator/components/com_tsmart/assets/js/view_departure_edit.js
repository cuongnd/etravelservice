    ...
    $.view_departure_default = function (element, options) {
        ...
        plugin.update_select_service_class = function () {
            ...
            $.each(list_service_class, function (index, item_service_class) {
                var $option = '<option  '+(item_service_class.tsmart_service_class_id==departure_item.tsmart_service_class_id?' selected ':'') +' value="' + item_service_class.tsmart_service_class_id + '">' + item_service_class.service_class_name + '</option>';
                $dialog_departure_edit_form.find('#tsmart_service_class_id').append($option);
            });
        ...
        plugin.init = function () {
            ...
            $element.find('.assign_user_id select.user').change(function () {
                ...
                var tsmart_departure_id=$tr.data('tsmart_departure_id');
                $.ajax({
                    type: "POST",
                    url: 'index.php',
                    dataType: "json",
                    data: (function () {
                        ...
                        dataPost = {
                            ...
                            task: 'ajax_save_departure_assign_user',
            ...
            $element.find('.edit-departure').click(function () {
                ...
                $.ajax({
                    ...
                    success: function (departure_item) {
                        ...
                        $('#tsmart_departure_parent_id').val(departure_item.tsmart_departure_parent_id);
                        plugin.fill_data();
...
*/
            ...
            $('.dialog-departure-edit-form').find('.save-close-price').click(function (e) {
                var departure_item=plugin.settings.departure_item;
                if(departure_item.tsmart_departure_id!=0)
                {
                    $('input[name="days_seleted"]').rules('remove', 'required');
