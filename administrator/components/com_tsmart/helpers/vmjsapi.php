	...
	static function jPrice() {
		...
		if(VmConfig::get('addtocart_popup',1)){
			...
			if(VmConfig::get('usefancy',1)){
				...
				$jsVars .= "vmCartText = '". addslashes( vmText::_('com_tsmart_CART_PRODUCT_ADDED') )."' ;\n" ;
				$jsVars .= "vmCartError = '". addslashes( vmText::_('com_tsmart_MINICART_ERROR_JS') )."' ;\n" ;
...
"  );
	...
	static public function vmValidator ($guest=null){
		...
		if (document.formvalidator.isValid(f)) {
				...
				var msg = '" .addslashes (vmText::_ ('com_tsmart_MISSING_REQUIRED_JS'))."';
			alert(msg + ' ');
		}
		return false;
	}";
	...
	static function jCreditCard()
	{
		...
		ccErrors [0] =  '" . addslashes( vmText::_('com_tsmart_CREDIT_CARD_UNKNOWN_TYPE') ). "';
		ccErrors [1] =  '" . addslashes( vmText::_("com_tsmart_CREDIT_CARD_NO_NUMBER") ). "';
		ccErrors [2] =  '" . addslashes( vmText::_('com_tsmart_CREDIT_CARD_INVALID_FORMAT')) . "';
		ccErrors [3] =  '" . addslashes( vmText::_('com_tsmart_CREDIT_CARD_INVALID_NUMBER')) . "';
		ccErrors [4] =  '" . addslashes( vmText::_('com_tsmart_CREDIT_CARD_WRONG_DIGIT')) . "';
		ccErrors [5] =  '" . addslashes( vmText::_('com_tsmart_CREDIT_CARD_INVALID_EXPIRE_DATE')) . "';
		";
	...
	static function jDate($date='',$name="date",$id=NULL,$resetBt = TRUE, $yearRange='',$options=array()) {
		...
		$dateFormat = vmText::_('com_tsmart_DATE_FORMAT_INPUT_J16');//="m/d/y"
		$search  = array('m', 'd', 'Y');
		...
		if ($date) {
			...
			$formatedDate = vmText::_('com_tsmart_NEVER');
		}
		...
		self::addJScript('datepicker','
			...
			jQuery(document).on( "click",".js-date-reset", function() {
				jQuery(this).prev("input").val("'.vmText::_('com_tsmart_NEVER').'").prev("input").val("0");
			});
		});
		');
	...
	static function date($date , $format ='LC2', $joomla=FALSE ,$revert=FALSE ){
		...
		if (!strcmp ($date, '0000-00-00 00:00:00')) {
			return vmText::_ ('com_tsmart_NEVER');
		}
		If ($joomla) {
			...
			$formatedDate = JHtml::_('date', $date, vmText::_('com_tsmart_DATE_FORMAT_'.$format.$J16));
		}
		return $formatedDate;
	}
