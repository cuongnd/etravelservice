...
class VmViewAdmin extends JViewLegacy
{
    ...
    */
    ...
    {
        return vmText::_('com_tsmart_FILTER') . ' ' . vmText::_($searchLabel) . ':
		<input type="text" name="' . $name . '" id="' . $name . '" value="' . $this->lists[$name] . '" class="text_area" />
		<button class="btn btn-small" onclick="this.form.submit();">' . vmText::_('com_tsmart_GO') . '</button>
		<button class="btn btn-small" onclick="document.getElementById(\'' . $name . '\').value=\'\';this.form.submit();">' . vmText::_('com_tsmart_RESET') . '</button>';
    }
    ...
    function addStandardEditViewCommands($id = 0, $object = null)
    {
        ...
        if ($editView and $id and (count(vmconfig::get('active_languages')) > 1)) {
            ...
            foreach ($languages as $k => &$joomlaLang) {
                if (!in_array($joomlaLang['value'], $activeVmLangs)) {
                    ...
                    if (!file_exists($image_flag)) {
                        vmerror(vmText::sprintf('com_tsmart_MISSING_FLAG', $image_flag, $joomlaLang['text']));
                    } else {
            ...
            if (isset($languagesByCode[$selectedLangue])) {
                ...
                vmWarn(vmText::sprintf('com_tsmart_MISSING_FLAG', $selectedLangue, $selectedLangue));
            }
    ...
    function SetViewTitle($name = '', $msg = '', $icon = '')
    {
        ...
        $viewText = vmText::_('com_tsmart_' . strtoupper($name));

        $taskName = ' <small><small>[ ' . vmText::_('com_tsmart_' . $task) . ' ]</small></small>';
    ...
    function toggle($field, $i, $toggle, $imgY = 'tick.png', $imgX = 'publish_x.png', $untoggleable = false)
    {
        ...
        if ($toggle == 'published') {
            ...
            $alt = $field ? vmText::_('com_tsmart_PUBLISHED') : vmText::_('com_tsmart_UNPUBLISHED');
            $action = $field ? vmText::_('com_tsmart_UNPUBLISH_ITEM') : vmText::_('com_tsmart_PUBLISH_ITEM');
        } else {
            $task = $field ? $toggle . '.0' : $toggle . '.1';
            $alt = $field ? vmText::_('com_tsmart_PUBLISHED') : vmText::_('com_tsmart_DISABLED');
            $action = $field ? vmText::_('com_tsmart_DISABLE_ITEM') : vmText::_('com_tsmart_ENABLE_ITEM');
        }
