...
// Ordering dropdown
	...
	static public function categoryListTree ($selectedCategories = array(), $cid = 0, $level = 0, $disabledFields = array()) {
		...
		if (empty(self::$categoryTree[$hash])) {
			...
			$cache = JFactory::getCache ('com_tsmart_cats');
			$cache->setCaching (1);
	...
	*/
	...
	static public function renderMetaEdit($obj){
		...
		$html = '<table>
					...
					'.VmHTML::row('textarea','com_tsmart_METAKEY','metakey',$obj->metakey,'class="inputbox"',80).'
					'.VmHTML::row('textarea','com_tsmart_METADESC','metadesc',$obj->metadesc,'class="inputbox"',80).'
					'.VmHtml::row('selectList','com_tsmart_METAROBOTS','metarobot',$obj->metarobot,$options).'
					'.VmHTML::row('input','com_tsmart_METAAUTHOR','metaauthor',$obj->metaauthor).'
				</table>';
		return $html;
	}
