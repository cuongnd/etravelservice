...
class VmMediaHandler {
	...
	function addMediaActionByType(){
		...
		$this->addMediaAction(0,'com_tsmart_NONE');

		$view = vRequest::getCmd('view');
		if($view!='media' || empty($this->file_name)){
			$this->addMediaAction('upload','com_tsmart_FORM_MEDIA_UPLOAD');
		}

		if(!empty($this->file_name)){
			$this->addMediaAction('replace','com_tsmart_FORM_MEDIA_UPLOAD_REPLACE');
			$this->addMediaAction('replace_thumb','com_tsmart_FORM_MEDIA_UPLOAD_REPLACE_THUMB');
		}
	...
	public function addMediaAttributesByType(){
		...
		if($this->setRole){
			// 				$this->addMediaAttributes('file_is_product_image','com_tsmart_FORM_MEDIA_SET_PRODUCT');
			$this->addMediaAttributes('product','com_tsmart_FORM_MEDIA_SET_PRODUCT'); // => file_is_displayable  =>location
			$this->addMediaAttributes('category','com_tsmart_FORM_MEDIA_SET_CATEGORY');
			$this->addMediaAttributes('manufacturer','com_tsmart_FORM_MEDIA_SET_MANUFACTURER');
			$this->addMediaAttributes('vendor','com_tsmart_FORM_MEDIA_SET_VENDOR');

			$this->_mRoles['file_is_displayable'] = 'com_tsmart_FORM_MEDIA_DISPLAYABLE' ;
			$this->_mRoles['file_is_downloadable'] = 'com_tsmart_FORM_MEDIA_DOWNLOADABLE' ;
			$this->_mRoles['file_is_forSale'] = 'com_tsmart_FORM_MEDIA_SET_FORSALE' ;
		} else {
	...
	public function displayFilesHandler($fileIds,$type,$vendorId = 0){
		...
		var medialink = '". JURI::root(false) ."administrator/index.php?option=com_tsmart&view=media&task=viewJson&format=json&mediatype=".$type."';
		...
		searchMedia.autocomplete({
			...
			select: function(event, ui){
				...
				//$(this).autocomplete( 'option' , 'source' , '". JURI::root(false) ."administrator/index.php?option=com_tsmart&view=product&task=getData&format=json&type=relatedcategories&row='+nextCustom )
...
$html .='</td>';
		...
		$html .= $this->displayRow('com_tsmart_FILES_FORM_FILE_TITLE','file_title');
		$html .= $this->displayRow('com_tsmart_FILES_FORM_FILE_DESCRIPTION','file_description');
		$html .= $this->displayRow('com_tsmart_FILES_FORM_FILE_META','file_meta');
		$html .= $this->displayRow('com_tsmart_FILES_FORM_FILE_CLASS','file_class');

		$html .= $this->displayRow('com_tsmart_FILES_FORM_FILE_URL','file_url',$readonly);
