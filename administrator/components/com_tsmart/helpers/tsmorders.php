...
class tsmorders
{
    const TYPE_GROUP_MAIN_TOUR_ORDER ='group_main_tour_order' ;
    ...
    const TYPE_GROUP_EXCURSION_ADD_ON_ORDER = 'group_excursion_add_on_order';
    /**
     * javascript strings
     *
     * @var    array
    ...
    public static function get_list_assign_user_id_manager_main_tour($tsmart_order_main_tour_id)
    {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select('user_id')
            ...
            ->where('type='.$query->q(self::TYPE_GROUP_MAIN_TOUR_ORDER))
        ;
        $list= $db->setQuery($query)->loadColumn();
        foreach($list as &$item){
            $item=(int)$item;
    ...
    public static function get_list_assign_user_id_manager_excusion_add_on_id($tsmart_order_transfer_addon_id)
    {
        ...
        $query->select('user_id')
            ->from('#__tsmart_order_user_manager')
            ->where('object_id='.(int)$tsmart_order_transfer_addon_id)
            ->where('type='.$query->q(self::TYPE_GROUP_EXCURSION_ADD_ON_ORDER))
        ;
        $list= $db->setQuery($query)->loadColumn();
        foreach($list as &$item){
            $item=(int)$item;
