	...
	static function jPrice() {
		...
		if(VmConfig::get('addtocart_popup',1)){
			...
			if(VmConfig::get('usefancy',1)){
				...
				$jsVars .= "vmCartText = '". addslashes( tsmText::_('com_tsmart_CART_PRODUCT_ADDED') )."' ;\n" ;
				$jsVars .= "vmCartError = '". addslashes( tsmText::_('com_tsmart_MINICART_ERROR_JS') )."' ;\n" ;
	...
	static function chosenDropDowns(){
		...
		if(!$chosenDropDowns){
			...
			if(VmConfig::get ('jchosen', 0) or $be){
				...
				$selectText = 'com_tsmart_DRDOWN_AVA2ALL';
				...
				if($be or vRequest::getInt('manage',false)){
					$selector = 'jQuery("select:not([disable_chosen=\"true\"])")';
				} else {
...
"  );
	...
	static public function vmValidator ($guest=null){
		...
		if (document.formvalidator.isValid(f)) {
				...
				var msg = '" .addslashes (tsmText::_ ('com_tsmart_MISSING_REQUIRED_JS'))."';
			alert(msg + ' ');
		}
		return false;
	}";
	...
	static function jCreditCard()
	{
		...
		ccErrors [0] =  '" . addslashes( tsmText::_('com_tsmart_CREDIT_CARD_UNKNOWN_TYPE') ). "';
		ccErrors [1] =  '" . addslashes( tsmText::_("com_tsmart_CREDIT_CARD_NO_NUMBER") ). "';
		ccErrors [2] =  '" . addslashes( tsmText::_('com_tsmart_CREDIT_CARD_INVALID_FORMAT')) . "';
		ccErrors [3] =  '" . addslashes( tsmText::_('com_tsmart_CREDIT_CARD_INVALID_NUMBER')) . "';
		ccErrors [4] =  '" . addslashes( tsmText::_('com_tsmart_CREDIT_CARD_WRONG_DIGIT')) . "';
		ccErrors [5] =  '" . addslashes( tsmText::_('com_tsmart_CREDIT_CARD_INVALID_EXPIRE_DATE')) . "';
		";
	...
	static function jDate($date='',$name="date",$id=NULL,$resetBt = TRUE, $yearRange='',$options=array()) {
		...
		$dateFormat = tsmText::_('com_tsmart_DATE_FORMAT_INPUT_J16');//="m/d/y"
		$search  = array('m', 'd', 'Y');
		...
		if ($date) {
			...
			$formatedDate = tsmText::_('com_tsmart_NEVER');
		}
		...
		self::addJScript('datepicker','
			...
			jQuery(document).on( "click",".js-date-reset", function() {
				jQuery(this).prev("input").val("'.tsmText::_('com_tsmart_NEVER').'").prev("input").val("0");
			});
		});
		');
	...
	static function date($date , $format ='LC2', $joomla=FALSE ,$revert=FALSE ){
		...
		if (!strcmp ($date, '0000-00-00 00:00:00')) {
			return tsmText::_ ('com_tsmart_NEVER');
		}
		If ($joomla) {
			$formatedDate = JHtml::_('date', $date, tsmText::_('DATE_FORMAT_'.$format));
		} else {

			$J16 = "_J16";

			$formatedDate = JHtml::_('date', $date, tsmText::_('com_tsmart_DATE_FORMAT_'.$format.$J16));
		}
		return $formatedDate;
	}
