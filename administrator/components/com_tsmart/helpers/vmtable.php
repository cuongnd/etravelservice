...
class VmTable extends vObject implements  JObservableInterface, JTableInterface {
	...
	public function setPrimaryKey($key, $keyForm = 0) {
		...
		$error = vmText::sprintf('com_tsmart_STRING_ERROR_PRIMARY_KEY', vmText::_('com_tsmart_' . strtoupper($key)));
...
//				vmdebug('setLoggableFieldsForStore ', $this->created_on);
	...
	public function jload($keys = null, $reset = true)
	{
    ...
    public function jstore($updateNulls = false)
    {
        $k = $this->_tbl_keys;
	...
	function check() {
		...
		foreach ($this->_obkeys as $obkeys => $error) {
			if (empty($this->$obkeys)) {
				$error = get_class($this) . ' ' .vmText::sprintf('com_tsmart_STRING_ERROR_OBLIGATORY_KEY', 'com_tsmart_' . strtoupper($obkeys) );
				vmError($error);
				return false;
			}
		}
		...
		if ($this->_unique) {
			...
			foreach ($this->_unique_name as $obkeys => $error) {
				...
				if (empty($this->$obkeys)) {
					$error = vmText::sprintf('com_tsmart_STRING_ERROR_NOT_UNIQUE_NAME', 'com_tsmart_' . strtoupper($obkeys));
