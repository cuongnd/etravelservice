...
class tsmilter{
	...
	public static function hl($t, $C=null, $S=array()){
		if ( is_string($C) ) $C = tsmilter::loadconfig($C);
		...
		$C['anti_link_spam'] = (isset($C['anti_link_spam']) && is_array($C['anti_link_spam']) && count($C['anti_link_spam']) == 2 && (empty($C['anti_link_spam'][0]) or tsmilter::hl_regex($C['anti_link_spam'][0])) && (empty($C['anti_link_spam'][1]) or tsmilter::hl_regex($C['anti_link_spam'][1]))) ? $C['anti_link_spam'] : 0;
		...
		if(!is_null(tsmilter::$C)){
			$reC = tsmilter::$C;
		}
		tsmilter::$C = $C;
		$S = is_array($S) ? $S : tsmilter::$hl_spec($S);
		if(!is_null(tsmilter::$S)){
			$reS = tsmilter::$S;
		}
		tsmilter::$S = $S;
		...
		if($C['unique_ids'] && is_null(tsmilter::$hl_Ids)){
			tsmilter::$hl_Ids = array();
		}
		if($C['hook']){
			$t = $C['hook']($t, $C, $S);
		}
		if($C['show_setting'] && preg_match('`^[a-z][a-z0-9_]*$`i', $C['show_setting'])){
			tsmilter::$C['show_setting'] = array('config'=>$C, 'spec'=>$S, 'time'=>microtime());
		}
		// main
		$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', 'vmFilter::hl_tag', $t);
		$t = $C['balance'] ? tsmilter::hl_bal($t, $C['keep_bad'], $C['parent']) : $t;
		$t = (($C['cdata'] or $C['comment']) && strpos($t, "\x01") !== false) ? str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05"), array('', '', '&', '<', '>'), $t) : $t;
		$t = $C['tidy'] ? tsmilter::hl_tidy($t, $C['tidy'], $C['parent']) : $t;
		unset($C, $e);
		if(isset($reC)){
			tsmilter::$C = $reC;
		}
	...
	public static function hl_bal($t, $do=1, $in='div'){
		...
		if(tsmilter::$C['direct_list_nest']){
	...
	public static function hl_spec($t){
		...
		for($i = count(($t = explode(';', $t))); --$i>=0;){
			...
			foreach(explode(',', $a) as $v){
				...
				if(isset($y[$x]['match']) && !tsmilter::hl_regex($y[$x]['match'])){
					unset($y[$x]['match']);
				}
				if(isset($y[$x]['nomatch']) && !tsmilter::hl_regex($y[$x]['nomatch'])){
					unset($y[$x]['nomatch']);
				}
			}
			if(!count($y) && !count($n)){
	...
	public static function hl_tag($t){
		...
		if($C['make_tag_strict'] && isset($eD[$e])){
			$trt = tsmilter::hl_tag2($e, $a, $C['make_tag_strict']);
			if(!$e){
				return (($C['keep_bad']%2) ? str_replace(array('<', '>'), array('&lt;', '&gt;'), $t) : '');
			}
		}
		...
		foreach($aA as $k=>$v){
			if(((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) or isset($rl[$k])) && ((!isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e])))){
				...
				if(isset($rl[$k]) && is_array($rl[$k]) && ($v = tsmilter::hl_attrval($v, $rl[$k])) === 0){
					continue;
				}
				$a[$k] = str_replace('"', '&quot;', $v);
			}
		...
		if($C['unique_ids'] && isset($a['id'])){
			if(!preg_match('`^[A-Za-z][A-Za-z0-9_\-.:]*$`', ($id = $a['id'])) or (!is_null(tsmilter::$hl_Ids[$id]) && $C['unique_ids'] == 1)){
				unset($a['id']);
			}else{
				while(!is_null(tsmilter::$hl_Ids[$id])){
					$id = $C['unique_ids']. $id;
				}
				tsmilter::$hl_Ids[($a['id'] = $id)] = 1;
			}
		}
		// xml:lang
