...
defined ('VMPATH_SITE') or define ('VMPATH_SITE', VMPATH_ROOT.DS.'components'.DS.'com_tsmart' );
defined ('VMPATH_ADMIN') or define ('VMPATH_ADMIN', VMPATH_ROOT.DS.'administrator'.DS.'components'.DS.'com_tsmart' );
 ...
 * So this works vmInfo('com_tsmart_MEDIA_NO_PATH_TYPE',$type,$link )
 * and also vmInfo('com_tsmart_MEDIA_NO_PATH_TYPE');
 *
 * @author Max Milbers
...
class tsmConfig {
	...
	public static $logFileName = 'com_tsmart';
	...
	static function echoAdmin(){
		if(self::$echoAdmin===FALSE){
			...
			if($user->authorise('core.admin','com_tsmart') or $user->authorise('core.manage','com_tsmart')){
				self::$echoAdmin = true;
			} else {
...
/**
	...
	static function set($key, $value){
		...
		if($admin = JFactory::getUser()->authorise('core.admin', 'com_tsmart')){
			if (!empty(self::$_jpConfig->_params)) {
	...
	public static function get_company_info()
	{
		...
		$company_info->company_name=$item->params->get('company_name',0);
...
class vmAccess {
	...
	static public function manager($task=0, $uid = 0, $and = false) {
		...
		if(!isset(self::$_manager[$h])) {
			...
			if($user->authorise('core.admin') or $user->authorise('core.admin', 'com_tsmart')) {
				self::$_manager[$h] = true;
			} else {
				self::$_manager[$h] = false;
				if(self::$_site){
					$a = $user->authorise('vm.manage', 'com_tsmart');
				} else {
					$a = $user->authorise('core.manage', 'com_tsmart');
				}
				...
				if($a){
					if(empty($task)){
						...
						foreach($task as $t){
							if($user->authorise('vm.'.$t, 'com_tsmart')){
								self::$_manager[$h] = true;
								if(!$and) break;
							}
							else if($and) {
