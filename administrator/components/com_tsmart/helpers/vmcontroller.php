...
class VmController extends JControllerLegacy{
	...
	public function __construct($cidName='cid', $config=array()) {
		...
		$this->mainLangKey = vmText::_('com_tsmart_'.strtoupper($this->_cname));
	...
	*/
	...
	function remove(){
		...
		$type = 'notice';
		if(count($ids) < 1) {
			$msg = vmText::_('com_tsmart_SELECT_ITEM_TO_DELETE');

		} else {
			$model = VmModel::getModel($this->_cname);
			$ret = $model->remove($ids);

			$msg = vmText::sprintf('com_tsmart_STRING_DELETED',$this->mainLangKey);
			if($ret==false) {
				$msg = vmText::sprintf('com_tsmart_STRING_COULD_NOT_BE_DELETED',$this->mainLangKey);
						$type = 'error';
			}
		}
	...
	public function toggle($field,$val=null){
		...
		if (!$model->toggle($field, $val, $this->_cidName, 0, $this->_cname)) {
			$msg = vmText::sprintf('com_tsmart_STRING_TOGGLE_ERROR',$this->mainLangKey);
		} else{
			$msg = vmText::sprintf('com_tsmart_STRING_TOGGLE_SUCCESS',$this->mainLangKey);
		}
	...
	public function publish($cidname=0,$table=0,$redirect = 0){
		...
		if (!$model->toggle('published', 1, $cidname, $table, $this->_cname)) {
			$msg = vmText::sprintf('com_tsmart_STRING_PUBLISHED_ERROR',$this->mainLangKey);
		} else{
			$msg = vmText::sprintf('com_tsmart_STRING_PUBLISHED_SUCCESS',$this->mainLangKey);
		}
	...
	function unpublish($cidname=0,$table=0,$redirect = 0){
		...
		if (!$model->toggle('published', 0, $cidname, $table, $this->_cname)) {
			$msg = vmText::sprintf('com_tsmart_STRING_UNPUBLISHED_ERROR',$this->mainLangKey);
		} else{
			$msg = vmText::sprintf('com_tsmart_STRING_UNPUBLISHED_SUCCESS',$this->mainLangKey);
		}
	...
	function orderup() {
		...
		$model = VmModel::getModel($this->_cname);
		$model->move(-1);
		$msg = vmText::sprintf('com_tsmart_STRING_ORDER_UP_SUCCESS',$this->mainLangKey);
		$this->setRedirect( $this->redirectPath, $msg);
	}
	...
	function orderdown() {
		...
		$model = VmModel::getModel($this->_cname);
		$model->move(1);
		$msg = vmText::sprintf('com_tsmart_STRING_ORDER_DOWN_SUCCESS',$this->mainLangKey);
		$this->setRedirect( $this->redirectPath, $msg);
	}
	...
	function saveorder() {
		...
		if (!$model->saveorder($cid, $order)) {
			...
			if(JFactory::getApplication()->isAdmin() and VmConfig::showDebug()){
				$msg = vmText::sprintf('com_tsmart_NEW_ORDERING_SAVEDF',$this->mainLangKey);
			} else {
				$msg = vmText::sprintf('com_tsmart_NEW_ORDERING_SAVED');
			}
