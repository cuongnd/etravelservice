 ...
 * @subpackage Currency
 */
class VirtueMartModelactivity extends VmModel {
	...
	function getListQuery()
	{
		...
		$query->select('activity.*')
			->from('#__virtuemart_activity AS activity')
		;
		...
		if ($search) {
			...
			$query->where('activity.title LIKE '.$search);
		}
	...
	function store(&$data){
		if(!vmAccess::manager('activity')){
			vmWarn('Insufficient permissions to store activity');
			return false;
		}
	...
	function remove($ids){
		if(!vmAccess::manager('activity')){
			vmWarn('Insufficient permissions to remove activity');
			return false;
		}
