 ...
 * @subpackage Currency
 */
class VirtueMartModelTourtype extends VmModel {
	...
	function store(&$data){
		if(!vmAccess::manager('tourtype')){
			vmWarn('Insufficient permissions to store tourtype');
			return false;
		}
	...
	function remove($ids){
		if(!vmAccess::manager('tourtype')){
			vmWarn('Insufficient permissions to remove tourtype');
			return false;
		}
