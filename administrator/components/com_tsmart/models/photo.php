 ...
 * @subpackage Currency
 */
class tsmartModelPhoto extends tmsModel
{
    ...
    function getListQuery()
    {
        ...
        $query->select('photo.*')
            ->from('#__tsmart_photo AS photo')
            ->where('photo.tsmart_product_id='.(int)$tsmart_product_id)
        ;
        ...
        if ($search) {
            ...
            $query->where('photo.photo_name LIKE ' . $search);
        }
    ...
    function store(&$data)
    {
        ...
        if ($file['name'] != "") {
            ...
            $new_file_upload_path = $path_upload_file_photo . $filename;
            $table_photo = tsmTable::getInstance('photo', 'Table');
            $table_photo->jload(array(file_upload => $new_file_upload_path));
            if ($table_photo->tsmart_photo_id) {
        ...
        if ($ok) {
            ...
            if ($file['name'] != "") {
                ...
                $src = $file['tmp_name'];
                $file_upload_path = $path_upload_file_photo . $filename;
                $dest = JPATH_ROOT . DS . $file_upload_path;

                $table_photo = tsmTable::getInstance('photo', 'Table');
                $table_photo->load($data['tsmart_photo_id']);
                $old_file_path = $table_photo->file_upload;
                ...
                $list_allow_image_type=tsmConfig::$_list_allow_image_type;

                if (in_array($ext_file,$list_allow_image_type )) {
    ...
    {
        if (!vmAccess::manager('photo')) {
            vmWarn('Insufficient permissions to remove photo');
            return false;
        }
