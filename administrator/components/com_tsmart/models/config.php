...
class tsmartModelConfig extends tmsModel {
	...
	function getFieldList($fieldname){
		...
		$dirs[] = VMPATH_ROOT.DS.'components'.DS.'com_tsmart'.DS.'sublayouts';
		...
		if($tplnames){
			if(is_dir(VMPATH_ROOT.DS.'templates'.DS.$tplnames.DS.'html'.DS.'com_tsmart'.DS.'sublayouts')){
				$dirs[] = VMPATH_ROOT.DS.'templates'.DS.$tplnames.DS.'html'.DS.'com_tsmart'.DS.'sublayouts';
			}
		}
	...
	static function getLayoutList($view,$ignore=0) {
		...
		if($com===0){
			...
			$dirs[] = VMPATH_ROOT.DS.'components'.DS.'com_tsmart'.DS.'views'.DS.$view.DS.'tmpl';
		...
		if($tplnames){
			if($com===0){
				...
				$opath = VMPATH_ROOT.DS.'templates'.DS.$tplnames.DS.'html'.DS.'com_tsmart'.DS.$view;
			}
			if(is_dir($opath)){
				$dirs[] = $opath;
			}
