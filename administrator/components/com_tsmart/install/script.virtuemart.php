...
// 			$this->path = JInstaller::getInstance()->getPath('extension_administrator');
		...
		public function preflight ($type, $parent=null) {
			...
			if(version_compare(JVERSION,'1.6.0','ge') and version_compare(JVERSION,'3.0.0','le')) {
				...
				$this->_db = JFactory::getDbo();
				$q = 'SELECT extension_id FROM #__extensions WHERE `type` = "component" AND `element` = "com_tsmart" ';
				$this->_db ->setQuery($q);
		...
		private function deleteSwfUploader(){
			if(JVM_VERSION>0){
				...
				if( JFile::exists(VMPATH_ROOT. DS. 'administrator' .DS. 'language' .DS. 'en-GB'. DS. 'en-GB.com_tsmart.sys.ini')){
					JFile::delete(VMPATH_ROOT. DS. 'administrator' .DS. 'language' .DS. 'en-GB'. DS. 'en-GB.com_tsmart.sys.ini');
				}
		...
		private function checkUserfields(){
			...
			, 'name' => 'tos'
			, 'title' => 'com_tsmart_STORE_FORM_TOS'
			, 'description' => ''
			, 'required' => 1
			, 'cart' => 1
			, 'account' => 0
