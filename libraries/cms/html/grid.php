...
abstract class JHtmlGrid
{
	...
	{
		return $checkedOut ? '' : '<label class="checkbox"><span class="cid">'.$recId.'</span><input type="checkbox" id="' . $stub . $rowNum . '" name="' . $name . '[]" value="' . $recId
			. '" onclick="Joomla.isChecked(this.checked);" /></label>';
	}
	...
	public static function edit($value,$key, $i,$link, $img = 'edit.png', $prefix = '')
	{
		return '<a href="'.$link.'" class="edit" data-id="'.$value->$key.'"  title="edit">'
			. JHtml::_('image', 'admin/' . $img, 'edit', null, true) . '</a>';
	}
	...
	public static function order($rows, $image = 'filesave.png', $task = 'saveorder')
	{
		return '<a href="javascript:saveorder('
			. (count($rows) - 1) . ', \'' . $task . '\')" rel="tooltip" class=" pull-right" title="'
