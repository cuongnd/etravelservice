 ...
 * Provides a modal file selector including upload mechanism
 *
 * @since  1.6
 */
class JFormFieldfile extends JFormField
{
	...
	/**
	 * Method to get the field input markup for a file selector.
	...
	protected function getInput()
	{
		...
		if (!self::$initialised)
		{
			...
			$script[] = '			}';
			$script[] = '			jfileRefreshPreview(id);';
			$script[] = '		}';
			$script[] = '	}';

			$script[] = '	function jfileRefreshPreview(id) {';
			$script[] = '		var $ = jQuery.noConflict();';
			...
			$script[] = '	function jfileRefreshPreviewTip(tip)';
			$script[] = '	{';
			$script[] = '		var $ = jQuery.noConflict();';
			$script[] = '		var $tip = $(tip);';
			$script[] = '		var $img = $tip.find("img.file-preview");';
			...
			$script[] = '	function jfileRefreshImgpathTip(tip)';
			$script[] = '	{';
			$script[] = '		var $ = jQuery.noConflict();';
			$script[] = '		var $tip = $(tip);';
		...
		if ($showPreview)
		{
			...
			$img = JHtml::image($src, JText::_('JLIB_FORM_file_PREVIEW_ALT'), $imgattr);
			$previewImg = '<div id="' . $this->id . '_preview_img"' . ($src ? '' : ' style="display:none"') . '>' . $img . '</div>';
			$previewImgEmpty = '<div id="' . $this->id . '_preview_empty"' . ($src ? ' style="display:none"' : '') . '>'
				. JText::_('JLIB_FORM_file_PREVIEW_EMPTY') . '</div>';

			if ($showAsTooltip)
			{
				$html[] = '<div class="file-preview add-on">';
				$tooltip = $previewImgEmpty . $previewImg;
				$options = array(
					'title' => JText::_('JLIB_FORM_file_PREVIEW_SELECTED_IMAGE'),
			...
			{
				$html[] = '<div class="file-preview add-on" style="height:auto">';
		...
		if ($this->disabled != true)
		{
			...
			$html[] = '<a class="modal btn" title="' . JText::_('JLIB_FORM_BUTTON_SELECT') . '" href="'
				. ($this->readonly ? ''
				: ($this->link ? $this->link
					: 'index.php?option=com_media&amp;view=files&amp;tmpl=component&amp;asset=' . $asset ) . '&amp;fieldid=' . $this->id . '&amp;folder=' . $folder) . '"'
